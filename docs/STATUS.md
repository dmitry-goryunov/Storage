...
monotonically with `vol` for optional exercise, and an independently-coded (plain nested
Python loops, no NumPy shift/broadcast shared with the production solver) reference
matches the vectorised solver exactly across four real-volatility, real-mean-reversion
configurations. The `n_p` convergence ladder is clean (0.023% to 0.0000% successive moves
from `n_p` 6 to 28); the `v_step` ladder found and fixed a real bug first -- `v_step=2000`
against `daily_max_mwh=1000` rounds to **zero** exercisable clips/day under Python's
banker's rounding, silently pricing an unexercisable swing with no error, the identical
defect class `normalise_storage_contract` exists to catch in the fixed-strike engine.
`ResetSwingTerms.__post_init__` now refuses that construction rather than rounding it away.
A full 12-month deal at production-adequate grid sizes runs in under a second, pure
Python/NumPy, no Numba needed yet. See
[`docs/DESIGN-MONTHLY-RESET-SWING-2026-09-13.md`](DESIGN-MONTHLY-RESET-SWING-2026-09-13.md)
for what's still open (the stricter brute-force scenario-tree enumeration, the averaged
reset). 238 tests pass (229 before this + 9).

**As of 2026-09-14.** No real term sheet exists for
this feature -- built as a generic prototype per an explicit user request ("just strike
price is changing month ahead, i.e. not fixed"), with every simplification named rather
than silently assumed (call-only, month-end point reset, a model-internal index, global
not per-month volume, no historical fixings). New: `reset_terms.py`
(`ResetSwingTerms`/`build_reset_schedule`), `reset_forward.py` (the conditional
month-ahead quote, by exact backward induction through the lattice's own transition
probabilities -- no derived closed form, avoiding the class of risk S5 already found
once), `reset_swing_exact.py::value_point_reset_call_swing` (the point-reset exact
benchmark, pure Python/NumPy per the design's own "small reference, not production
scale"). `storage_model.py::Storage.build()` gained three lines exposing `p_u`/`p_m`/`p_d`
on the instance; `storage_kernels.py` untouched. Pinned at near-zero volatility, where
the answer is hand-computable: single-month mandatory, two-month global-quota chaining
across the month rotation, and optional-volume touching zero-margin days not at all --
all three matched by construction, not by tuning. See
[`docs/DESIGN-MONTHLY-RESET-SWING-2026-09-13.md`](DESIGN-MONTHLY-RESET-SWING-2026-09-13.md)
sec.13 for exactly what remains (exhaustive-enumeration cross-check, convergence ladders,
the averaged reset). 229 tests pass (198 before this + 31 here).

**As of 2026-09-11 (unified pricing app feature).** The single-contract Streamlit
app now accepts complete put-swing, call-swing and storage term sheets. Storage
inputs include independent days-to-fill and days-to-empty, exact opening/terminal
inventory, variable costs, injection fuel, ratchets, a ratchet grid-loss gate and
dated inventory bounds; swing inputs include strike, maximum total/daily volume
and optional call volume. The app uses the shared content-cleaned quote path,
shows the exact grid it actually prices, separates physical forward delta from
PV-tailed futures delta, and exports an audit JSON plus curve, schedule and hedge
CSVs. A Streamlit-independent boundary in `pricing_app_core.py` refuses any
term sheet that the selected grid would silently change. Verification on the
current feature tree is **194 passed**, including 11 new focused app-boundary
checks and real-widget runs through all three product paths.
