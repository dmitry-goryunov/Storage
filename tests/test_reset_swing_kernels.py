...
`n_extra_fixing_days` (scenarios below) puts EXTRA leading days into
`fixing_observation_dates` that are not in `exercise_dates` -- the case
2026-09-14 INDEPENDENT-REVIEW-MONTHLY-RESET-SWING R-02 found silently
mishandled (a partial delivery month's own preceding fixing window reaching
back further than that month's own exercise days). Several scenarios below
use `n_extra_fixing_days=0` (the degenerate case where the two calendars
coincide, matching a complete delivery month) specifically because that WAS
the only case the pre-fix kernel got right -- keeping it pins that the fix
did not regress the common case while the >0 scenarios pin the fix itself.
"""
import numpy as np
import pandas as pd
import pytest
...
def _scenario(n_p, n_l, n_r, n_exercise_days, n_extra_fixing_days, daily_max_clips, seed):
    rng = np.random.default_rng(seed)
    val_date = pd.Timestamp("2026-01-01")
    fixing = pd.Timestamp("2026-02-28")
    fixing_window = pd.date_range(
        pd.Timestamp("2026-03-01") - pd.Timedelta(days=n_extra_fixing_days),
        periods=n_extra_fixing_days + n_exercise_days, freq="D")
    exercise_dates = fixing_window[n_extra_fixing_days:]
    end_date = fixing_window[-1] + pd.Timedelta(days=10)
    ...
    month = rt.DeliveryMonth(label=None, fixing_date=fixing, exercise_dates=tuple(exercise_dates),
                             fixing_observation_dates=tuple(fixing_window))
    ...
    fixing_indices = np.array([date_span.get_loc(d) for d in fixing_window], dtype=np.int64)
    n_exercise_days_out = len(exercise_dates)
    return dict(lattice=lattice, month=month, r_grid=r_grid,
               ...
               fixing_observation_dates=tuple(fixing_window),
               fixing_indices=fixing_indices, n_exercise_days=n_exercise_days_out)


@pytest.mark.parametrize(
    "n_p,n_l,n_r,n_exercise_days,n_extra_fixing_days,daily_max_clips,seed", [
    (1, 2, 5, 1, 0, 1, 0),     # smallest possible: single exercise day, single clip, no extra
    (2, 3, 8, 2, 0, 2, 1),     # complete-month degenerate case (extra=0)
    (3, 4, 10, 3, 2, 1, 2),    # partial month: 2 fixing-only days ahead of 3 exercise days
    (5, 6, 12, 5, 1, 3, 3),    # daily_max_clips >= n_l - 1: exercises the `break` in the clip loop
    (8, 5, 15, 4, 3, 6, 4),    # daily_max_clips > n_l: same break, more aggressively; extra=3
    (1, 8, 6, 7, 0, 1, 5),     # smallest width (n_p=1 -> 3), larger n_l and more days, no extra
    (4, 4, 9, 1, 6, 2, 6),     # extreme: almost the whole window is fixing-only, one exercise day
])
def test_kernel_matches_the_python_reference_exactly(
        n_p, n_l, n_r, n_exercise_days, n_extra_fixing_days, daily_max_clips, seed):
    s = _scenario(n_p, n_l, n_r, n_exercise_days, n_extra_fixing_days, daily_max_clips, seed)
...
def test_kernel_is_substantially_faster_at_realistic_month_scale():
    ...
    s = _scenario(n_p=15, n_l=25, n_r=50, n_exercise_days=30, n_extra_fixing_days=0,
                  daily_max_clips=2, seed=7)
    lattice = s["lattice"]
